window.wxMiniGame = function (exports, Laya) {
	...
	class MiniFileMgr {
	    ...
	    static writeFilesList(fileurlkey, filesListStr, isAdd) {
	        ...
	        if (!MiniAdpter.isZiYu && MiniAdpter.isPosMsgYu) {
	            MiniAdpter.window.tt.postMessage({ url: fileurlkey, data: MiniFileMgr.filesListObj[fileurlkey], isLoad: "filenative", isAdd: isAdd });
	        }
	    }
	    ...
	    static setNativeFileDir(value) {
	        MiniFileMgr.fileNativeDir = MiniAdpter.window.tt.env.USER_DATA_PATH + value;
	    }
	}
	...
	class MiniSound extends Laya.EventDispatcher {
	    ...
	    static _createSound() {
	        MiniSound._id++;
	        return MiniAdpter.window.tt.createInnerAudioContext();
	    }
	    load(url) {
	...
	class MiniInput {
	    ...
	    static _createInputElement() {
	        ...
	        MiniAdpter.window.tt.onWindowResize && MiniAdpter.window.tt.onWindowResize(function (res) {
	        });
	    ...
	    static wxinputFocus(e) {
	        ...
	        MiniAdpter.window.tt.offKeyboardConfirm();
	        MiniAdpter.window.tt.offKeyboardInput();
	        MiniAdpter.window.tt.showKeyboard({ defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"] || 'done', success: function (res) {
	            }, fail: function (res) {
	            } });
	        MiniAdpter.window.tt.onKeyboardConfirm(function (res) {
	        ...
	        MiniAdpter.window.tt.onKeyboardInput(function (res) {
	            var str = res ? res.value : "";
	    ...
	    static hideKeyboard() {
	        MiniAdpter.window.tt.offKeyboardConfirm();
	        MiniAdpter.window.tt.offKeyboardInput();
	        MiniAdpter.window.tt.hideKeyboard({ success: function (res) {
	                console.log('隐藏键盘');
	            }, fail: function (res) {
	...
	class MiniLoader extends Laya.EventDispatcher {
	    ...
	    _loadResourceFilter(type, url) {
	        var thisLoader = this;
	        if (url.indexOf(MiniAdpter.window.tt.env.USER_DATA_PATH) == -1 && (url.indexOf("http://") != -1 || url.indexOf("https://") != -1)) {
	            if (MiniFileMgr.loadPath != "") {
	                url = url.split(MiniFileMgr.loadPath)[1];
	            }
	            else {
	    ...
	    _loadSound(url) {
	        ...
	        if (MiniFileMgr.isLocalNativeFile(url)) {
	            ...
	            if (!MiniFileMgr.isLocalNativeFile(url) && (tempurl.indexOf("http://") == -1 && tempurl.indexOf("https://") == -1) || (tempurl.indexOf(MiniAdpter.window.tt.env.USER_DATA_PATH) != -1)) {
	                MiniLoader.onDownLoadCallBack(url, thisLoader, 0);
	            }
	            else {
	    ...
	    _loadHttpRequestWhat(url, contentType) {
	        ...
	        if (Laya.Loader.preLoadedMap[url])
	            ...
	            var tempurl = Laya.URL.formatURL(url);
	            if (url.indexOf(MiniAdpter.window.tt.env.USER_DATA_PATH) == -1 && (tempurl.indexOf("http://") != -1 || tempurl.indexOf("https://") != -1) && !MiniAdpter.AutoCacheDownFile) {
	    ...
	    static onReadNativeCallBack(url, type = null, thisLoader = null, errorCode = 0, data = null) {
	        if (!errorCode) {
	            ...
	            if (!MiniAdpter.isZiYu && MiniAdpter.isPosMsgYu && type != Laya.Loader.BUFFER) {
	                MiniAdpter.window.tt.postMessage({ url: url, data: tempData, isLoad: "filedata" });
	            }
	    ...
	    static _transformImgUrl(url, type, thisLoader) {
	        ...
	        if (!MiniFileMgr.getFileInfo(url)) {
	            var tempUrl = Laya.URL.formatURL(url);
	            if (url.indexOf(MiniAdpter.window.tt.env.USER_DATA_PATH) == -1 && (tempUrl.indexOf("http://") != -1 || tempUrl.indexOf("https://") != -1)) {
	                if (MiniAdpter.isZiYu) {
	                    thisLoader._loadImage(url);
	                }
	                else {
	...
	class MiniLocalStorage {
	    ...
	    static setItem(key, value) {
	        try {
	            MiniAdpter.window.tt.setStorageSync(key, value);
	        }
	        catch (error) {
	            MiniAdpter.window.tt.setStorage({
	                key: key,
	                data: value
	            });
	        }
	    ...
	    static getItem(key) {
	        return MiniAdpter.window.tt.getStorageSync(key);
	    }
	    ...
	    static removeItem(key) {
	        MiniAdpter.window.tt.removeStorageSync(key);
	    }
	    static clear() {
	        MiniAdpter.window.tt.clearStorageSync();
	    }
	    static getStorageInfoSync() {
	        try {
	            var res = MiniAdpter.window.tt.getStorageInfoSync();
	...
	class MiniAdpter {
	    ...
	    static init(isPosMsg = false, isSon = false) {
	        ...
	        MiniAdpter.systemInfo = MiniAdpter.window.tt.getSystemInfoSync();
	        ...
	        MiniAdpter.window.CanvasRenderingContext2D.prototype = MiniAdpter.window.tt.createCanvas().getContext('2d').__proto__;
	        ...
	        MiniAdpter.window.tt.onMessage && MiniAdpter.window.tt.onMessage(MiniAdpter._onMessage);
	    }
	    ...
	    static createElement(type) {
	        if (type == "canvas") {
	            ...
	            if (MiniAdpter.idx == 1) {
	                ...
	                _source = MiniAdpter.window.tt.createCanvas();
	            }
	            MiniAdpter.idx++;
	            return _source;
	        }
	    ...
	    static postInfoToContext(url, atlaspngUrl, atlasJson) {
	        ...
	        if (fileNativeUrl) {
	            MiniAdpter.window.tt.postMessage({ url: Laya.URL.formatURL(url), atlasdata: postData, imgNativeUrl: fileNativeUrl, imgReadyUrl: textureUrl, isLoad: "opendatacontext" });
	        }
	        else {
	    ...
	    static sendSinglePicToOpenDataContext(url) {
	        ...
	        if (fileNativeUrl) {
	            MiniAdpter.window.tt.postMessage({ url: url, imgNativeUrl: fileNativeUrl, imgReadyUrl: url, isLoad: "openJsondatacontextPic" });
	        }
	        else {
	            throw "获取图集的磁盘url路径不存在！";
	        }
	    ...
	    static sendJsonDataToDataContext(url) {
	        if (!MiniAdpter.isZiYu) {
	            ...
	            if (atlasJson) {
	                MiniAdpter.window.tt.postMessage({ url: url, atlasdata: atlasJson, isLoad: "openJsondatacontext" });
	            }
	            else {
	                throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
	            }
	...
	class MiniAccelerator extends Laya.EventDispatcher {
	    ...
	    static startListen(callBack) {
	        ...
	        try {
	            MiniAdpter.window.tt.onAccelerometerChange(MiniAccelerator.onAccelerometerChange);
	        }
	        catch (e) { }
	    }
	    static stopListen() {
	        ...
	        try {
	            MiniAdpter.window.tt.stopAccelerometer({});
	        }
	        catch (e) { }
	    }
	...
	class MiniLocation {
	    ...
	    static getCurrentPosition(success = null, error = null, options = null) {
	        ...
	        paramO.fail = error;
	        MiniAdpter.window.tt.getLocation(paramO);
	...
	class MiniVideo {
	    constructor(width = 320, height = 240) {
	        this.videoend = false;
	        this.videourl = "";
	        this.videoElement = MiniAdpter.window.tt.createVideo({ width: width, height: height, autoplay: true });
	    }
	    static __init__() {
	    }
